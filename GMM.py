...
# -*- coding: utf-8 -*-
"""
Created on Wed May 23 09:40:11 2020
...
class GaussianMixture:
    ...
    def calc_score(self, X, ci):  ### formula 9 in exp form
        ...
        if self.coefs[ci] > 0:
            ...
            mult = np.einsum(
                'ij,ij->i', diff, np.dot(np.linalg.inv(self.covariances[ci]), diff.T).T) # formaula 9 2nd
    ...
    def calc_prob(self, X):
        prob = [self.calc_score(X, ci) for ci in range(self.n_components)] # formula 8
